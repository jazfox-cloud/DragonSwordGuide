...
try {
  ...
  cdp.socket.addEventListener('message', ({ data }) => {
    ...
    if (e.method === 'Network.requestWillBeSent' && /google-analytics\.com\/g\/collect/.test(e.params.request.url)) {
      ...
      for (const line of (e.params.request.postData || '').split(/\r?\n/)) {
  ...
  // Attempt debug tagging without replacing gtag or adding another config/ID.
  // Always label this run TEST_VALIDATION_ONLY; do not assume reporting exclusion.
  ...
  let documentCollectionStart = 0;
  const navigatePage = async url => {
    documentCollectionStart = collections.length;
    await navigate(cdp, url);
  };
  const settle = async () => {
    await delay(250);
    if (!remoteBaseUrl) return;
    // Wait for real async gtag delivery before navigating away from this document.
    for (let i=0;i<40;i++) {
      const pending = await events();
      if (pending.length && pending.every(x => collections.slice(documentCollectionStart).filter(c=>c.event===x[1]&&responses.some(r=>r.requestId===c.requestId&&r.status===204)).length >= pending.filter(y=>y[1]===x[1]).length)) return;
      await delay(500);
    }
    throw new Error('Timed out awaiting this document analytics receipt; do not navigate away and lose queued events');
  };
  ...
  await navigatePage(baseUrl + '/map/'); await settle();
  ...
  await navigatePage(baseUrl+'/map/?marker='+encodeURIComponent(marker.id)); await settle();
  ...
  if (!remoteBaseUrl) {
    ...
    await navigatePage(baseUrl + '/map/?marker=marker:chest:orbis-castle-approach:02'); await settle();
    ev = await events();
  ...
  await navigatePage(baseUrl+'/map/');await settle();ev=await events();
  ...
  if(remoteBaseUrl){
    ...
    // Debug delivery is best-effort, reported separately from transport acceptance.
  ...
  const result={kind:'TEST_VALIDATION_ONLY',debugModeConfirmed:collections.filter(c=>c.event?.startsWith('tool_')||c.event==='map_marker_completed').every(c=>c.debug==='1')&&collections.length>0,baseUrl,at:new Date().toISOString(),checks,failures,runtimeErrors,collections:collections.map(({requestId,...c})=>({...c,status:responses.find(r=>r.requestId===requestId)?.status})),organicUsageClaim:false};
